...
async function runReconstruction(source, scene, boneGroupRef, onProgress, onDone) {
  try {
    ...
    const boneMat = new THREE.MeshStandardMaterial({
      ...
      color: 0xE0CCА0, roughness: 0.55, metalness: 0.03,
      envMapIntensity: 0.45,
    });
    // Fix bad hex — use numeric
    boneMat.color.setHex(0xE0CCA0);

    const cartMat = new THREE.MeshPhysicalMaterial({
      color: 0xAADDFF, roughness: 0.12, metalness: 0.0,
      transparent: true, opacity: 0.7,
      transmission: 0.4, thickness: 0.15, ior: 1.35,
      emissive: new THREE.Color(0x002040), emissiveIntensity: 0.15,
